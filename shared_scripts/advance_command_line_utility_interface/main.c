...
#include<iostream>
namespace errors {
  void report_and_throw(const std::string error) {
...
namespace read_options {
  ...
  namespace for_single_string {
    ...
    void read_more_ops(input_ops & older_ops, std::string_view options) {
      ...
      older_ops.append_range(more_options);
    }
    input_ops read_args(command_line_option option, std::string_view options) {
      ...
      result.push_back(options_and_option_args {
        option,
        args
      } );
      read_more_ops(result, options);
      return result;
    }
    input_ops read(std::string_view options) {
      if (options.length() == 0) {
  ...
  namespace for_multiple_strings {
    ...
    void read_more_ops(input_ops & older_ops, int argc, char * argv[]) {
      input_ops more_options = read_interface(argc - 2, argv + 2);
      older_ops.append_range(more_options);
    }
    input_ops read_args(command_line_option option, int argc, char * argv[]) {
      ...
      read_more_ops(result, argc, argv);
      return result;
    }
  ...
  bool use_single_string_funcs(std::string_view string_in_question) {
    return string_in_question.find_first_of(' ') != std::string_view::npos;
  }
  input_ops for_multiple_strings::read_interface(int argc, char * argv[]) {
    ...
    if (argv[0][0] == '-') {
      if (use_single_string_funcs(argv[0])) {
        return for_multiple_strings::read(argc, argv);
      } else {
        return for_single_string::read(std::string_view {
          argv[0] + 1
        });
      }
    } else {
